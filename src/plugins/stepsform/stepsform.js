...
var componentName = "wb-steps",
	...
	init = function( evt ) {
		...
		if ( elm ) {
			...
			btnPrevious = createStepsButton( "BUTTON", "prev", "mrgn-rght-sm mrgn-bttm-md", i18nText.prv );
			btnNext = createStepsButton( "BUTTON", "next", "mrgn-bttm-md", i18nText.nxt );
	...
	setStepsBtnEvent = function( elm ) {
		elm.addEventListener( "click", function( evt ) {
			...
			// continue if valid
			if ( isFormValid ) {
				showSteps( this.parentElement.parentElement, isNext );
				if ( isNext ) {
	...
	showSteps = function( elm, isNext ) {
		var fields = elm.getElementsByTagName( "FIELDSET" )[ 0 ].getElementsByTagName( "div" )[ 0 ],
			legend = elm.getElementsByTagName( "FIELDSET" )[ 0 ].getElementsByTagName( "legend" )[ 0 ],
